...
public class BoardApp {
	...
	public void start() {
		...
		while (true) {
			...
			count++;
			if(count==3) {
				System.out.println("로그인 세번실패 했습니다 비밀번호를 받을 이메일을 입력하세요 >> ");
				String pwMail = scn.nextLine();
				SendMail mail = new SendMail();
				mail.sendMail(pwMail, "새로운 비밀번호", pwMail+"님의 새 비밀번호는 1212");
				uservice.initPassword(new User(id, "1212", "")); //pw초기화
				count=0;
				continue;
			}
